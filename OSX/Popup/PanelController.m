...
- (int)startUserList
{
    _userList = true;
    if (!_userListThread) {
        _userListThread = [[NSThread alloc] initWithTarget:self selector:@selector(userListThread) object:nil ];
        [_userListThread start];
    } else if ([_userListThread isFinished])
    [_userListThread start];
    return (1);
}
...
{
    if (_userListThread) {
        [_userListThread cancel];
        [_userListLoading setHidden:YES];
        [_userListLoading stopAnimation:self];
        _userListThread = nil;
#if LOG
        NSLog(@"[UserList] Service stoped.");
#endif
    }
    _userList = false;
}
...
- (void)userListThread
{
    NSData *rawData = nil;
    NSError *error;
    NSMutableArray *rawJson;
    
#if LOG
    NSLog(@"[UserList] Starting Thread.");
#endif
    while (_userList)
    {
        [_userListLoading setHidden:NO];
        [_userListLoading startAnimation:self];
        
        rawData = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:@"http://jobertomeu.fr/bmtech/servlet/listConnected.php"]];
        rawJson = [NSJSONSerialization JSONObjectWithData:rawData options:NSJSONReadingMutableContainers error:&error];
        if (error) {
            NSLog(@"%@", [error localizedDescription]);
        } else {
            NSLog(@"NBRRR => %@", [rawJson valueForKey:@"nbr_connected"]);
            if ([NSNull null] != [rawJson valueForKey:@"connected"]) {
                NSArray *list = [rawJson valueForKey:@"connected"];
                [_loginList removeAllItems];
                for (NSDictionary *user in list )
                {
                    [_loginList addItemWithObjectValue:user[@"login"]];
                    [user dealloc];
                }
            } else {
                NSLog(@"Connected not found");
            }
        }
...
#if LOG
    NSLog(@"[UserList] Thread terminated.");
#endif
}
...
- (IBAction)browseClick:(id)sender {
    ...
    [panel setCanChooseDirectories:NO]; //FOR MOMENT
    ...
    if (clicked == NSFileHandlingPanelOKButton) {
        for (NSURL *url in [panel URLs]) {
            NSLog(@"URL => %s", [url fileSystemRepresentation]);
        }
    }
}
